...
import { list, get, create, update, remove, publish } from '../services/assignment.service';
...
const FIELDS = [
  ...
  { name: 'lateSubmissionAllowed', label: 'Late Submission Allowed', type: 'checkbox' },
];
  ...
  { key: 'active', label: 'Active', render: (r) => (r.active ? 'Yes' : 'No') },
  {
    key: 'actions', label: 'Actions', render: (row) => (
      <div className="flex flex-wrap gap-1">
        ...
        <button onClick={() => doPublish(row.id)} className="rounded border px-2 py-1 text-xs hover:bg-gray-100">Publish</button>
...
export default function Assignments() {
  ...
  const viewFields = useMemo(() => [
    ...
    { key: 'dueDate', label: 'Due Date' }, { key: 'totalMarks', label: 'Total Marks' },
    { key: 'fileUrl', label: 'File URL' }, { key: 'status', label: 'Status' },
    { key: 'lateSubmissionAllowed', label: 'Late Submission', render: (v) => (v ? 'Yes' : 'No') },
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={openCreate} className="rounded-md bg-navy-primary px-3 py-1.5 text-sm font-medium text-white hover:bg-navy-hover">New Assignment</button>
      </div>
      <input type="search" placeholder="Search assignments..." className="w-full max-w-sm rounded-md border border-gray-300 px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-navy-primary" onChange={(e) => setSearchTerm(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && fetchPage(0)} />
      ...
      <EntityFormModal open={openModal} title={editing ? 'Edit Assignment' : 'New Assignment'} fields={FIELDS} initialValues={editing || {}} onSubmit={onSubmit} onClose={() => { setOpenModal(false); setEditing(null); }} loading={formLoading} />
      <ViewModal open={!!viewing} title="Assignment Details" record={viewing} fields={viewFields} onClose={() => setViewing(null)} />
    </div>
  );
}
