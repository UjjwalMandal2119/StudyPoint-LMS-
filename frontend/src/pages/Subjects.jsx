...
const COLS = [
  ...
  { key: 'practical', label: 'Practical', render: (r) => (r.practical ? 'Yes' : 'No') },
...
export default function Subjects() {
  ...
  const viewFields = useMemo(() => [
    ...
    { key: 'description', label: 'Description' }, { key: 'courseId', label: 'Course ID' },
    { key: 'courseName', label: 'Course Name' }, { key: 'teacherId', label: 'Teacher ID' },
    { key: 'teacherName', label: 'Teacher Name' }, { key: 'totalMarks', label: 'Total Marks' },
    { key: 'passMarks', label: 'Pass Marks' }, { key: 'practical', label: 'Practical', render: (v) => (v ? 'Yes' : 'No') },
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={openCreate} className="rounded-md bg-indigo-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-indigo-700">New Subject</button>
      </div>
      <input type="search" placeholder="Search subjects..." className="w-full max-w-sm rounded-md border border-gray-300 px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400" onChange={(e) => setSearchTerm(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && fetchPage(0)} />
      ...
      <ViewModal open={!!viewing} title="Subject Details" record={viewing} fields={viewFields} onClose={() => setViewing(null)} />
    </div>
  );
}
