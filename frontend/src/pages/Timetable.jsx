...
import { list, get, create, update, remove, toggleActive } from '../services/timetable.service';
...
const FIELDS = [
  ...
  { name: 'dayOfWeek', label: 'Day of Week', type: 'select', required: true, options: ['MONDAY','TUESDAY','WEDNESDAY','THURSDAY','FRIDAY','SATURDAY','SUNDAY'] },
...
export default function Timetable() {
  ...
  const viewFields = useMemo(() => [
    { key: 'id', label: 'ID' }, { key: 'batchName', label: 'Batch' },
    { key: 'subjectName', label: 'Subject' }, { key: 'teacherName', label: 'Teacher' },
    { key: 'dayOfWeek', label: 'Day of Week' }, { key: 'startTime', label: 'Start Time' },
    { key: 'endTime', label: 'End Time' }, { key: 'roomNumber', label: 'Room Number' },
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={openCreate} className="rounded-md bg-navy-primary px-3 py-1.5 text-sm font-medium text-white hover:bg-navy-hover">New Entry</button>
      </div>
      ...
      <EntityFormModal open={openModal} title={editing ? 'Edit Timetable' : 'New Timetable'} fields={FIELDS} initialValues={editing || {}} onSubmit={onSubmit} onClose={() => { setOpenModal(false); setEditing(null); }} loading={formLoading} />
      <ViewModal open={!!viewing} title="Timetable Details" record={viewing} fields={viewFields} onClose={() => setViewing(null)} />
    </div>
  );
}
