...
const SLIDES = [
  {
    ...
    gradient: 'from-[#7b1113] via-[#8e1b1e] to-[#5e0d0f]',
    emoji: '🎓',
    badge: 'Learn Smarter',
  },
  {
    ...
    gradient: 'from-[#7b1113] via-[#a33a3d] to-[#c1595c]',
    emoji: '🚀',
  ...
  },
  {
    ...
    gradient: 'from-[#5e0d0f] via-[#7b1113] to-[#a33a3d]',
    emoji: '📊',
    badge: 'Grow Together',
  },
];
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-white selection:bg-[#7b1113]/20">
      {/* ============ NAVBAR ============ */}
      <nav className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${navSolid ? 'bg-white/95 shadow-lg backdrop-blur' : 'bg-transparent'}`}>
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-3">
          <a href="#home" className="flex items-center gap-2">
            <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-[#7b1113] to-[#5e0d0f] text-lg text-white">📘</span>
            <span className={`text-xl font-extrabold ${navSolid ? 'text-gray-900' : 'text-white'}`}>
              Study<span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-200 to-[#7b1113]">Point</span>
            </span>
          </a>
          <div className="hidden items-center gap-1 md:flex">
            {NAV_LINKS.map((l) => (
              <a key={l.label} href={l.href} className={`rounded-full px-4 py-2 text-sm font-medium transition hover:bg-white/20 ${navSolid ? 'text-gray-700 hover:bg-[#7b1113]/5 hover:text-[#7b1113]' : 'text-white'}`}>
                {l.label}
              </a>
            ))}
          </div>
          <div className="hidden items-center gap-2 md:flex">
            <Link to="/login" className={`rounded-full px-4 py-2 text-sm font-semibold transition ${navSolid ? 'text-gray-700 hover:text-[#7b1113]' : 'text-white hover:bg-white/20'}`}>Login</Link>
            <Link to="/register" className="rounded-full bg-gradient-to-r from-[#7b1113] to-[#5e0d0f] px-5 py-2 text-sm font-semibold text-white shadow-lg shadow-[#7b1113]/30 transition hover:scale-105">Get Started</Link>
          </div>
        ...
        {mobileOpen && (
          <div className="space-y-1 bg-white px-6 pb-4 md:hidden">
            {NAV_LINKS.map((l) => (
              <a key={l.label} href={l.href} onClick={() => setMobileOpen(false)} className="block rounded-lg px-3 py-2 text-sm font-medium text-gray-700 hover:bg-[#7b1113]/5">{l.label}</a>
            ))}
            <div className="flex gap-2 pt-2">
              <Link to="/login" onClick={() => setMobileOpen(false)} className="flex-1 rounded-lg border border-[#7b1113]/30 py-2 text-center text-sm font-semibold text-[#7b1113]">Login</Link>
              <Link to="/register" onClick={() => setMobileOpen(false)} className="flex-1 rounded-lg bg-[#7b1113] py-2 text-center text-sm font-semibold text-white">Get Started</Link>
            </div>
          </div>
        )}
      </nav>
          ...
          >
            <div className={`flex min-h-screen flex-col items-center justify-center bg-gradient-to-br ${s.gradient} px-6 text-center`}>
              ...
              <div className="mt-9 flex flex-wrap justify-center gap-4">
                <Link to="/register" className="rounded-full bg-white px-8 py-3 font-semibold text-[#7b1113] shadow-xl transition hover:scale-105">Get Started Free</Link>
          ...
          <div className="grid grid-cols-2 gap-4 rounded-3xl bg-white p-8 shadow-2xl ring-1 ring-gray-100 md:grid-cols-4">
            {STATS.map((s) => (
              <div key={s.label} className="text-center">
                <p className="bg-gradient-to-r from-[#7b1113] to-[#5e0d0f] bg-clip-text text-4xl font-extrabold text-transparent">{s.value}</p>
      ...
      <section id="about" className="mx-auto max-w-7xl px-6 py-20">
        <div className="text-center">
          <span className="inline-block rounded-full bg-[#7b1113]/10 px-4 py-1.5 text-sm font-semibold text-[#7b1113]">About Study Point</span>
          <h2 className="mt-4 text-3xl font-extrabold text-gray-900 md:text-5xl">A Complete <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#7b1113] to-[#5e0d0f]">Learning</span> Ecosystem</h2>
      ...
      <section id="vision" className="mx-auto max-w-7xl px-6 pb-20">
        <div className="grid gap-6 md:grid-cols-2">
          <div className="group rounded-3xl bg-gradient-to-br from-[#7b1113] to-[#5e0d0f] p-8 text-white shadow-xl transition hover:-translate-y-1">
          ...
          <div className="group rounded-3xl bg-gradient-to-br from-[#7b1113] to-[#c1595c] p-8 text-white shadow-xl transition hover:-translate-y-1">
            <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-white/20 text-3xl">🎯</div>
      ...
      <section id="features" className="bg-gradient-to-b from-gray-50 to-white py-20">
        <div className="mx-auto max-w-7xl px-6">
          <div className="text-center">
            <span className="inline-block rounded-full bg-[#7b1113]/10 px-4 py-1.5 text-sm font-semibold text-[#7b1113]">Features</span>
          ...
          <div className="mt-12 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {FEATURES.map((f) => (
              <div key={f.title} className="group rounded-3xl border border-gray-100 bg-white p-7 shadow-sm transition hover:-translate-y-1 hover:border-[#7b1113]/20 hover:shadow-xl">
                <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-[#7b1113]/15 to-[#5e0d0f]/15 text-3xl transition group-hover:scale-110">{f.emoji}</div>
      ...
      {/* ============ QUOTATIONS / MISSION ============ */}
      <section id="mission" className="relative overflow-hidden bg-gradient-to-br from-slate-900 via-[#3d0a0b] to-slate-900 py-20">
        <div className="pointer-events-none absolute -left-16 -top-16 h-64 w-64 rounded-full bg-[#7b1113]/30 blur-3xl" />
        <div className="pointer-events-none absolute -bottom-16 -right-16 h-64 w-64 rounded-full bg-[#7b1113]/50/30 blur-3xl" />
        ...
        <div className="grid gap-8 md:grid-cols-2">
          ...
          <div className="rounded-3xl border border-gray-100 bg-white p-8 shadow-xl">
            <form onSubmit={(e) => e.preventDefault()} className="space-y-4">
              <input required placeholder="Your name" className="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#7b1113]" />
              <input required type="email" placeholder="Your email" className="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#7b1113]" />
              <textarea required rows={4} placeholder="Your message" className="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#7b1113]" />
              <button className="w-full rounded-xl bg-gradient-to-r from-[#7b1113] to-[#5e0d0f] py-3 font-semibold text-white shadow-lg transition hover:scale-[1.01]">Send Message</button>
            </form>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-6 pb-20">
        <div className="relative overflow-hidden rounded-[2rem] bg-gradient-to-r from-[#7b1113] via-[#8e1b1e] to-[#5e0d0f] px-8 py-16 text-center text-white shadow-2xl">
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-4">
            <Link to="/register" className="rounded-full bg-white px-8 py-3 font-semibold text-[#7b1113] shadow-xl transition hover:scale-105">Create Free Account</Link>
      ...
      <footer className="border-t border-gray-100 bg-gray-50">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 px-6 py-10 md:flex-row">
          <div className="flex items-center gap-2">
            <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-[#7b1113] to-[#5e0d0f] text-white">📘</span>
            <span className="text-lg font-extrabold text-gray-900">Study<span className="text-transparent bg-clip-text bg-gradient-to-r from-[#7b1113] to-[#5e0d0f]">Point</span></span>
          </div>
          <nav className="flex flex-wrap justify-center gap-x-6 gap-y-2 text-sm text-gray-600">
            {NAV_LINKS.map((l) => (
              <a key={l.label} href={l.href} className="transition hover:text-[#7b1113]">{l.label}</a>
            ))}
          </nav>
